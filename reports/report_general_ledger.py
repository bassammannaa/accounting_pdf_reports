...
class ReportGeneralLedgerExcel(models.AbstractModel):
    _name = 'report.accounting_pdf_reports.report_generalledger_excel'
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, 
        l.currency_id, l.amount_currency,
        concat(l.ref, ' ' , hm.external_office_id,' ', hm.full_name, ' ', ofic."name", ' ' , hremp."name")  AS lref, hm.external_office_id,
        l.name AS lname, COALESCE(l.debit,0) AS debit, 
        COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
            m.name AS move_name, c.symbol AS currency_code, p.name AS partner_name\
            FROM account_move_line l\
    ...
    def generate_xlsx_report(self, workbook, data, wizard):
        try:
            ...
            # for account in accounts:
            current_row = 0
            sheet = workbook.add_worksheet("%s - %s" % (accounts.code, accounts.name))

            sheet.write(current_row, 0, 'Transaction Date', header_line_format_string)
            sheet.set_column(0, 0, 20)

            sheet.write(current_row, 1, 'Move', header_line_format_string)
            ...
            sheet.write(current_row, 5, 'Debit (KWD)', header_line_format_num)
            sheet.set_column(5, 5, 15)

            sheet.write(current_row, 6, 'Credit (KWD)', header_line_format_num)
            sheet.set_column(6, 6, 15)

            sheet.write(current_row, 7, 'Balance (KWD)', header_line_format_num)
